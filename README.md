...
*   **Encryption**: Keys are stored securely in your browser's local storage (`localStorage`) encrypted with AES-256-GCM derived from your PIN.
*   **Privacy**: Your PIN is never stored in plain text. Only a salted hash is saved in your browser's local storage (`localStorage`) for verification.
